...
async function updateSheetByEmail(email) {
  ...
  // Find most recent row with this email that is pending
  let rowIndex = -1;
...
async function createICountReceipt({ clientName, clientEmail, amount, currency, description }) {
  if (!ICOUNT_TOKEN) { console.log('iCount token not set, skipping receipt'); return; }
...
module.exports = async function handler(req, res) {
  ...
  const name = order?.user_name || order?.first_order_item?.product_name || '';
  const total = order?.total;
  const currency = order?.currency?.toUpperCase() || 'ILS';
  const amount = total ? (parseInt(total) / 100).toFixed(2) : '0';

  console.log('order:', { email, name, amount, currency });
