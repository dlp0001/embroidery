...
- Состав занятия делится надвое. «Ждём» — те, кого записал родитель или у
  ...
  на занятие.
- Про каждого нужно знать ровно четыре вещи: был или нет и как оплачено —
  по абонементу, наличными или никак. По умолчанию «не оплачено».
  «По абонементу» показываем только когда занятие на пакет уже село или
  родитель записал человека и пакет у семьи есть. Гадать по наличию
  абонемента нельзя, от этого статус скакал. Наличные ставятся нажатием на
  строчку про деньги и заводят платёж с провайдером `cash`; повторное
  нажатие его удаляет и возвращает занятие в абонемент.
